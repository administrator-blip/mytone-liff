...
<head>
    ...
    <!-- ビルド識別子（Service Worker キャッシュとの不一致確認用。デプロイ時に手動更新） -->
    <meta name="mytone-build" content="2026-05-04-01">
    ...
    <style>
        ...
        *::after {
        ...
        .loading-progress .lp-bar {
            height: 100%;
            background: linear-gradient(90deg, #5ba88c, #6dbfa3, #e8cc6b);
            background-size: 200% 100%;
            border-radius: 3px;
            width: 0%;
            transition: width 0.3s linear;
            animation: lp-gradient 2s linear infinite;
        }
        ...
        .loading-progress .lp-sub {
            font-size: 0.75rem;
            color: #999;
        }

        .loading-progress .lp-elapsed {
            font-size: 0.72rem;
            color: #aaa;
            margin-top: 4px;
        }
        ...
        body.dark .loading-progress .lp-message {
            color: #8ec5b0;
        }

        body.dark .loading-progress .lp-bar-wrap {
            background: rgba(91, 168, 140, 0.2);
        }

        body.dark .loading-progress .lp-sub {
            color: #777;
        }

        body.dark .loading-progress .lp-elapsed {
            color: #666;
        }
...
<body>
    ...
    <div id="screenDiag" class="container hidden">
        <div class="tab-bar" id="diagTabs">
            <div class="tab" onclick="switchDiagTab('book')">📝 予約を入れる</div>
        ...
        <!-- 診断士用予約入力エリア -->
        <div id="diagBook" class="hidden">
            <div class="card">
                <div class="card-title"><span class="ct-icon">📝</span> 予約を入れる</div>
                <!-- 担当店舗選択（複数店舗の場合のみ選択肢が出る） -->
                <div id="diagSalonWrap">
                    <label>担当店舗</label>
                    <select id="diagSalonSelect" onchange="STATE.diag_salon_id=this.value"></select>
                </div>
                <button class="btn btn-primary" onclick="diagStartNewBooking()" style="margin-top:14px">➕
                    新規予約を入力する</button>
                <div class="msg-info" style="margin-top:10px;font-size:0.82rem"><b>STEP1</b>（初回診断）として登録されます。</div>
            </div>
        </div>
        <div id="diagAdd">
            <div class="card">
        ...
        <div id="diagList" class="hidden">
            <div class="card">
                ...
                <div style="
                    ...
                    line-height: 1.5;
                ">
                    ℹ️ 表示は本日から<b>60日分</b>（約2ヶ月先まで）です。<br>
                    ※ 予約のお客様データは期間に関わらず全件表示されます。
                </div>
                <!-- 稼働枠カレンダービュー -->
    ...
    <script>
        ...
        const STATE = {
            line_user_id: '',
            id_token: '',         // LIFF ID Token (GAS側で検証してuserIdをspoof対策)
            role: '',
            salon_id: '',
            diag_id: '',
            diag_salons: [],      // 診断士の担当店舗リスト（apiBootstrapから取得）
            diag_salon_id: '',    // 診断士が予約入力時に選択した店舗ID
            step_type: '',
            parent_booking_id: '',
            preferred_diag_id: '',
            customer: {},
        ...
        /* ========== 設定: ここだけ変更する ==========
         * デフォルト値は本番。ステージング等で切り替えたい場合は
         * URLパラメータ ?liff=XXX&gas=YYY で上書き可能。
         */
        const DEFAULT_LIFF_ID = '2009132853-rlGuhhMj';
        const DEFAULT_GAS_URL = 'https://script.google.com/macros/s/AKfycbyJ6RasshdyfQVueQvWu58MIS-ILVXDwxiK-6m4lmCmhV28m_ebOOhwhWLL36yW3JJ-mg/exec';

        // ID Token認証スイッチ（GAS側のLINE_LOGIN_CHANNEL_ID設定確認後にtrueへ）
        // - false: id_tokenを送信しない（既存のline_user_idベース動作 = 後方互換）
        // - true:  id_tokenを送信、GAS側で検証してuserIdを上書き（強化）
        const ENABLE_ID_TOKEN_AUTH = false;

        // URLパラメータからの上書きをサポート（ステージング・デバッグ用）
        const _urlParams = new URLSearchParams(location.search);
        const LIFF_ID = _urlParams.get('liff') || DEFAULT_LIFF_ID;
        const GAS_URL = _urlParams.get('gas') || DEFAULT_GAS_URL;
        if (_urlParams.has('liff') || _urlParams.has('gas')) {
            console.warn('[mytone] config overridden via URL params');
        }
        ...
        async function initLiff() {
            ...
            try {
                ...
                // LIFF ID Token を取得（GAS側で検証され、認証済みuserIdが利用される）
                try {
                    STATE.id_token = liff.getIDToken() || '';
                    console.log('id_token: ' + (STATE.id_token ? 'obtained' : 'not available'));
                } catch (idErr) {
                    console.warn('getIDToken failed:', idErr);
                    STATE.id_token = '';
                }
            ...
            callApi('apiBootstrap', {
                ...
                , (res) => {
                    ...
                    if (res.role === 'staff') {
                        ...
                        // 担当店舗リストを保存（予約入力機能で使用）
                        STATE.diag_salons = res.salons || [];
                        if (STATE.diag_salons.length > 0) {
                            STATE.diag_salon_id = STATE.diag_salons[0].salon_id;
                        }
                        // デバッグ情報を保存（salonsが空のとき診断画面に表示する）
                        STATE.bootstrap_debug = res.debug || null;
                        if (res.debug) console.log('[apiBootstrap.debug]', res.debug);
                        setBadge('診断士');
                        show('screenDiag');
                        initDiagUI();
                    }
        ...
        function doSearchSlots() {
            ...
            // SWRキャッシュキー（検索条件の組み合わせ）
            const CACHE_KEY = 'mytone_slots_' + STATE.step_type + '_' + ymd + '_' + (diagFilter || 'all');

            // SWR: キャッシュがあれば即座に表示（待ち時間ゼロ体験）
            const cached = _loadCache(CACHE_KEY);
            if (cached && cached.results) {
                if (cached.mode === 'no_preferred_slot') {
                    el('slotsArea').innerHTML = '<div class="msg-warn">同一診断士の空きがありません</div>';
                    show('btnFallback');
                } else {
                    renderSlots(cached.results, 'slotsArea');
                ...
                if (area) area.insertAdjacentHTML('afterbegin', '<div style="font-size:0.75rem;color:#888;text-align:center;padding:4px 0">🔄 最新情報を取得中...</div>');
            } else {
                showLoadingProgress('slotsArea', { message: '空き枠を検索中...', estimateSec: 4, icon: '🔍' });
            }

            // ボタン多重押し防止
            const searchBtn = document.querySelector('#dateSearch .btn-primary');
            disableBtn(searchBtn, '検索中...');
            ...
            // 診断士ロール時はsalon_idを付加
            if (STATE.role === 'diagnostician' && STATE.diag_salon_id) {
                payload.salon_id = STATE.diag_salon_id;
            }

            // バックグラウンドで最新データを取得
            callApi('apiSearchSlots', payload, (res) => {
                if (!res.ok) {
                    if (!cached) el('slotsArea').innerHTML = '<div class="msg-error">' + esc(res.error) + '</div>';
                    else showToast('更新失敗: ' + res.error, true);
                    enableBtn(document.querySelector('#dateSearch .btn-primary'));
                    return;
                }

                // キャッシュに保存（TTL: 3分 — スロットはリアルタイム性重要なため短め）
                _saveCache(CACHE_KEY, res);
        ...
        function doSearchFallback() {
            const ymd = el('searchDate').value;
            showLoadingProgress('slotsArea', { message: '別の診断士を検索中...', estimateSec: 4, icon: '🔍' });
            hide('btnFallback');
            const fbBtn = el('btnFallback');
            if (fbBtn) disableBtn(fbBtn, '検索中...');
        ...
        function doListBookings() {
            ...
            showLoadingProgress('bookingsList', { message: '予約一覧を取得中...', estimateSec: 3, icon: '📋' });
        ...
        function doSearchChangeSlots() {
            ...
            showLoadingProgress('changeSlotsArea', { message: '変更先の空き枠を検索中...', estimateSec: 4, icon: '🔄' });
            hide('btnConfirmChange');
        ...
        /**
         * プログレスバー付きローディングを表示
         * @param {string} targetId - 表示先のDOM要素ID
         * @param {object} opts - { message: 表示テキスト, estimateSec: 推定秒数, icon: アイコン }
         * @returns {string} タイマーID（stopLoadingProgressで使用）
         */
        function showLoadingProgress(targetId, opts) {
            opts = opts || {};
            const message = opts.message || '読み込み中...';
            const estimateSec = opts.estimateSec || 4;
            const icon = opts.icon || '🔍';
            const timerId = 'lp_' + targetId + '_' + Date.now();

            const html = `<div class="loading-progress" id="${timerId}">
                <div class="lp-icon">${icon}</div>
                <div class="lp-message">${esc(message)}</div>
                <div class="lp-bar-wrap"><div class="lp-bar" id="${timerId}_bar"></div></div>
                <div class="lp-sub">通常 約${estimateSec}秒で完了します</div>
                <div class="lp-elapsed" id="${timerId}_elapsed"></div>
            </div>`;
            ...
            let phaseReached = 0; // 0=通常, 1=遅延, 2=タイムアウト

            const intervalId = setInterval(() => {
                const elapsedSec = (Date.now() - startTime) / 1000;
                const barEl = document.getElementById(timerId + '_bar');
                const elapsedEl = document.getElementById(timerId + '_elapsed');
                const msgEl = document.getElementById(timerId);

                if (!barEl || !msgEl) {
                    clearInterval(intervalId);
                    delete _progressTimers[timerId];
                    return;
                }

                // プログレスバー: 推定時間で90%に到達、最大95%
                const progress = Math.min(95, (elapsedSec / estimateSec) * 90);
                barEl.style.width = progress + '%';

                // 経過表示
                if (elapsedEl) {
                    elapsedEl.textContent = Math.floor(elapsedSec) + '秒経過';
                }

                // 5秒超: メッセージ変更
                if (elapsedSec > 5 && phaseReached < 1) {
                    phaseReached = 1;
                    const lpMsg = msgEl.querySelector('.lp-message');
                    if (lpMsg) lpMsg.textContent = '少々お待ちください…';
                    const lpSub = msgEl.querySelector('.lp-sub');
                    if (lpSub) lpSub.textContent = 'サーバーと通信しています';
                }

                // 15秒超: タイムアウト表示
                if (elapsedSec > 15 && phaseReached < 2) {
                    phaseReached = 2;
                    clearInterval(intervalId);
                    delete _progressTimers[timerId];
                    el(targetId).innerHTML = `<div class="msg-warn" style="text-align:center">
                        <div style="font-size:1.2rem;margin-bottom:6px">⏱️</div>
                        <div style="font-weight:600;margin-bottom:4px">応答に時間がかかっています</div>
                        <div style="font-size:0.8rem;color:#888">ネットワーク状況をご確認ください</div>
                    </div>`;
                }
            }, 300);

            _progressTimers[timerId] = intervalId;
            return timerId;
        }

        /**
         * プログレスローディングを停止（API完了時に呼ぶ）
         */
        function stopLoadingProgress(timerId) {
            if (timerId && _progressTimers[timerId]) {
                clearInterval(_progressTimers[timerId]);
                delete _progressTimers[timerId];
            }
        }
         ...
         * ボタンをローディング状態にする
         */
        function disableBtn(btnEl, loadingText) {
            if (!btnEl) return;
        ...
        function enableBtn(btnEl) {
            ...
            if (btnEl._origText) btnEl.textContent = btnEl._origText;
        }
        ...
        function doViewHistory() {
            ...
            showLoadingProgress('historyContent', { message: '過去の予約を取得中...', estimateSec: 3, icon: '📞' });
            openHistorySheet();
        ...
        function initDiagUI() {
            ...
            // 担当店舗セレクトを初期化（診断士が予約入力機能で使用）
            const salonSel = el('diagSalonSelect');
            const salonWrap = el('diagSalonWrap');
            if (salonSel) {
                if (STATE.diag_salons.length > 0) {
                    const sOpts = STATE.diag_salons.map(s =>
                        `<option value="${esc(s.salon_id)}">${esc(s.salon_name)}</option>`
                    );
                    salonSel.innerHTML = sOpts.join('');
                    STATE.diag_salon_id = STATE.diag_salons[0].salon_id;
                } else {
                    // 担当店舗が0件: デバッグ情報を表示
                    if (salonWrap) {
                        let dbgHtml = '<div class="msg-warn">担当店舗が見つかりません。下記情報を管理者に共有してください</div>';
                        if (STATE.bootstrap_debug) {
                            const d = STATE.bootstrap_debug;
                            const matchCount = (d.matching_rows || []).length;
                            dbgHtml += '<div style="font-size:0.78rem;background:#f5f5f5;padding:8px;margin-top:6px;border-radius:6px;font-family:monospace;word-break:break-all">'
                                + '<b>診断士ID:</b> "' + esc(d.searched_diag_id) + '" (長さ:' + d.searched_diag_id_len + ')<br>'
                                + '<b>salon_diag 全行数:</b> ' + d.salon_diag_total_rows + '<br>'
                                + '<b>このIDに一致した行:</b> ' + matchCount + '件<br>';
                            if (matchCount > 0) {
                                dbgHtml += '<b>一致行詳細:</b><br>';
                                for (const m of d.matching_rows) {
                                    dbgHtml += '&nbsp;&nbsp;salon_id=' + esc(m.salon_id)
                                        + ', active=' + esc(JSON.stringify(m.active_raw))
                                        + ' (' + esc(m.active_type) + ')<br>';
                                }
                            }
                            dbgHtml += '<b>salon_diag内の登録diag_id総数:</b> ' + (d.unique_diag_id_count_in_salon_diag || 0) + '<br>';
                            if (d.sample_diag_ids && d.sample_diag_ids.length) {
                                dbgHtml += '<b>サンプル（先頭3件）:</b><br>'
                                    + d.sample_diag_ids.map(x => '&nbsp;&nbsp;"' + esc(x) + '"').join('<br>');
                            }
                            dbgHtml += '</div>';
                        }
                        salonWrap.innerHTML = dbgHtml;
                    }
                    // 予約入力ボタンを無効化
                    const bookBtn = document.querySelector('#diagBook .btn-primary');
                    if (bookBtn) {
                        bookBtn.disabled = true;
                        bookBtn.textContent = '担当店舗未登録のため利用不可';
                    }
                }
            }
        }
        ...
        function switchDiagTab(tab) {
            ...
            if (tab === 'book') {
                tabs[0].classList.add('active');
                show('diagBook'); hide('diagAdd'); hide('diagList'); hide('diagDash');
            } else if (tab === 'add') {
                tabs[1].classList.add('active');
                hide('diagBook'); show('diagAdd'); hide('diagList'); hide('diagDash');
            } else if (tab === 'list') {
                tabs[2].classList.add('active');
                hide('diagBook'); hide('diagAdd'); show('diagList'); hide('diagDash');
                doListAvail();
            } else if (tab === 'dash') {
                tabs[3].classList.add('active');
                hide('diagBook'); hide('diagAdd'); hide('diagList'); show('diagDash');
                loadDiagDashboard();
            }
        }

        /* ========== 診断士ロール: 新規予約入力 ========== */
        function diagStartNewBooking() {
            if (!STATE.diag_salon_id) {
                showToast('店舗を選択してください', true);
                return;
            }
            // スタッフ用フローの頑客情報入力フォームを再利用
            // screenStaff正表ではなく、診断士ダッシュボード「予約を入れる」タブ内にスタッフ用フォームを表示
            // screenStaffを一時的に表示して誤注点を回避するため、封じ込めます
            STATE.step_type = 'STEP1';
            STATE.parent_booking_id = '';
            STATE.preferred_diag_id = '';
            STATE.customer = {};
            STATE.selectedSlot = null;
            STATE.selectedDiagId = '';
            STATE.selectedDiagName = '';
            // 診断士予約: screenStaff 内の custInfo フォームを再利用する
            // 注意: custInfo は staffNew の子要素なので staffNew は表示したまま、
            // 中身（stepSelect / step2 / dateSearch / confirmArea / todaySummary）を全て隠す
            hide('screenDiag');
            show('screenStaff');
            hide('staffTabs');     // スタッフ用タブは診断士フローでは混乱の元なので隠す
            // staffNew は custInfo の親なので表示したまま、兄弟要素のみ隠す
            hide('todaySummary');
            hide('stepSelect');
            hide('step2ParentSearch');
            hide('dateSearch');
            ...
            hide('changeArea');
            show('custInfo');
            el('custStepLabel').textContent = '📝 STEP1 初回診断（診断士入力）';
            el('custName').value = '';
            el('custPhone').value = '';
            el('custGender').value = '';
            el('custAge').value = '';
            if (el('custMemo')) el('custMemo').value = '';
            // 戻るボタンの動作を診断士タブに戻るよう上書き
            const backBtn = document.querySelector('#custInfo .btn-outline');
            if (backBtn) backBtn.setAttribute('onclick', 'diagCancelBooking()');
        }

        /* 診断士予約フローを途中でキャンセルし診断士UIに戻る */
        function diagCancelBooking() {
            hide('screenStaff');
            show('screenDiag');
            // 隠していたスタッフ用タブを復元（次回スタッフロールで利用する場合のため）
            show('staffTabs');
            hide('custInfo');
            show('stepSelect');
            switchDiagTab('book');
            // 戻るボタンの動作を元に戻す
            const backBtn = document.querySelector('#custInfo .btn-outline');
            if (backBtn) backBtn.setAttribute('onclick', 'resetNewBooking()');
        }

        /* ========== 診断士ダッシュボード ========== */
        function loadDiagDashboard() {
            const CACHE_KEY = 'mytone_dash_cache_' + STATE.diag_id;

            // SWR: キャッシュがあれば即座に表示
            const cached = _loadCache(CACHE_KEY);
            if (cached) {
                _renderDiagDash(cached);
                const listEl = el('diagBookingsList');
                if (listEl) listEl.insertAdjacentHTML('afterbegin', '<div id="dashRefreshBadge" style="font-size:0.75rem;color:#888;text-align:center;padding:4px 0">🔄 最新情報を取得中...</div>');
            } else {
                showLoadingProgress('diagBookingsList', { message: '予約情報を取得中...', estimateSec: 3, icon: '📅' });
            }

            callApi('apiListBookings', { line_user_id: STATE.line_user_id, q_phone: '', q_name: '' }, (res) => {
                if (!res.ok) {
                    if (!cached) el('diagBookingsList').innerHTML = '<div class="msg-error">' + esc(res.error) + '</div>';
                    else showToast('更新失敗: ' + res.error, true);
                    return;
                }
                _saveCache(CACHE_KEY, res.bookings || []);
                _renderDiagDash(res.bookings || []);
            });
        }

        function _renderDiagDash(allBookings) {
            const myBookings = allBookings.filter(b => b.diag_id === STATE.diag_id && b.status !== 'CANCELED');
            const now = new Date();
            ...
            for (const b of sorted) {
                ...
                parts.push(`<div class="dash-booking">
            ...
            el('diagBookingsList').innerHTML = parts.join('');
        }

        // SWRキャッシュユーティリティ（TTL: 5分）
        ...
        function _loadCache(key) {
            try {
                ...
                // TTL内（5分）ならキャッシュを使用
                if (Date.now() - obj.t < _CACHE_TTL) return obj.d;
        ...
        function doListAvail() {
            const CACHE_KEY = 'mytone_avail_cache_' + STATE.diag_id;

            // SWR: キャッシュがあれば即座に表示（待ち時間ゼロ体験）
            const cached = _loadCache(CACHE_KEY);
            if (cached) {
                _renderAvailList(cached);
                const listEl = el('availList');
                if (listEl) listEl.insertAdjacentHTML('afterbegin', '<div style="font-size:0.75rem;color:#888;text-align:center;padding:4px 0">🔄 最新情報を取得中...</div>');
            } else {
                showLoadingProgress('availList', { message: '稼働枠を取得中...', estimateSec: 3, icon: '🗓️' });
            }

            // バックグラウンドで最新データを取得
            callApi('apiDiagListAvail', { line_user_id: STATE.line_user_id }, (res) => {
                if (!res.ok) {
                    if (!cached) el('availList').innerHTML = '<div class="msg-error">' + esc(res.error) + '</div>';
                    else showToast('更新失敗: ' + res.error, true);
                    return;
                }
                _saveCache(CACHE_KEY, res.avail);
                _renderAvailList(res.avail);
            });
        }

        function _renderAvailList(avail) {
            renderAvailCalendar(avail);
            if (avail.length === 0) {
            ...
            for (const a of avail) {
                parts.push(`<div class="avail-item"><div class="avail-time">📅 ${fmtDt(a.start_at)} 〜 ${fmtTime(a.end_at)}</div><button class="btn btn-danger btn-small" onclick="doDeleteAvail('${esc(a.event_id)}')">削除</button></div>`);
            }
            el('availList').innerHTML = parts.join('');
        }
        ...
        function callApi(fn, payload, cb) {
            // body に id_token を同梱（GAS側で検証して line_user_id を上書き）
            const body = { action: fn, payload: payload };
            if (ENABLE_ID_TOKEN_AUTH && STATE.id_token) body.id_token = STATE.id_token;
        ...
        /* ========== ビルド識別子の表示（キャッシュ不一致確認用） ========== */
        (function showBuildBadge() {
            const meta = document.querySelector('meta[name="mytone-build"]');
            const build = meta ? meta.getAttribute('content') : 'unknown';
            ...
            console.log('[mytone] build:', build);
        })();
